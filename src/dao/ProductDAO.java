...
public class ProductDAO {
    ...
    public List<Product> getActiveProducts() {
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            while (rs.next()) {
                products.add(new Product(rs.getInt("id"), rs.getInt("seller_id"), rs.getString("title"),
    ...
    public List<Product> searchProducts(String keyword) {
        List<Product> products = new ArrayList<>();
        String sql = "SELECT id, seller_id, title, description, price, status FROM products WHERE status = 'ACTIVE' AND (title LIKE ? OR description LIKE ?)";
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
